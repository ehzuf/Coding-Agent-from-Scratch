...
> **教学简化**：Claude Code 的只读命令验证（`readOnlyCommandValidation.ts` + `readOnlyValidation.ts`）超过 1800 行，不仅检查命令名，还对每个命令的参数做深度校验（例如 `git` 的只读子命令列表、`find` 是否带 `-delete` 参数等）。我们这里使用简单的命令名白名单，满足教学演示需求。
